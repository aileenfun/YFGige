...
void CUsbControlDlg::OnBnClickedsendpic()
{
	CString strFile = _T("");

	//CFileDialog    dlgFile(TRUE, NULL, NULL, OFN_HIDEREADONLY, _T("Image files(*.bmp)|*.bmp|All Files (*.*)|*.*||"), NULL);
	...
	while(1)
	{
		...
		cv::Mat sendimg = cv::imread(strStd[i]);
		//cv::namedWindow("sendfile", cv::WINDOW_NORMAL);
		//cv::resizeWindow("sendfile", 1280, 1024);
		//cv::moveWindow("sendfile", 0, 0);

		cv::cvtColor(sendimg, sendimg, cv::COLOR_BGR2GRAY);
		//cv::imshow("sendfile", sendimg);
		int contin = sendimg.isContinuous();
		if (contin)
			GigESendFile((char*)sendimg.data, sendimg.cols * sendimg.rows, sendimg.cols, sendimg.rows);
		i++;
		if (i == 3)
		{
			i = 0;
